...
page_count: 17
extracted_at: "2026-02-07T05:30:19.409731+00:00"
status: "success"
---
...
| 366 | Journal of Business & Economic Statistics, October 2005 |
| --- | --- |
| main complications in composite hypotheses testing is that (as- | of possible decision rules. Decisions are evaluated with a real- |
| ymptotic) distributions typically depend on nuisance parame- | is a random variable valued loss function, L(ξt, δk,t−h), where ξt |
| ters, such that the null distribution is not unique. The usual way | that represents the aspects of the decision problem that are un- |
| to handle this ambiguity is to use the least favorable conﬁg- | known at the time that the decision is made. We evaluate fore- |
| uration (LFC), which is sometimes referred to as “the point | casts in terms of their expected loss, E[L(ξt, δk,t−h)]. Thus we |
| least favorable to the alternative.” Our analysis shows that the | need not assume that any of the forecasts are constructed from |
| LFC-based approach leads to some rather unfortunate prop- | a correctly speciﬁed model. Whenever δk,t−h = δk,t−h( ˆθk,t−h) |
|  | ˆ |
| erties when testing for SPA. The following situation delivers | these are likely to in- is based on estimated parameters, θk,t−h, |
| key insight to the advantages of using a sample-dependent null | ﬂuence the expected loss—typically by increasing the expected |
| distribution. Let pmin denote the smallest p value of the m pair- | loss. We make assumptions that do not permit parameters that |
| wise comparisons (comparing each alternative with the bench- | are estimated with the recursive scheme. However, the rolling |
| mark); then the Bonferroni bound test (at level α) rejects the | scheme is accommodated by our framework, and so is the ﬁxed |
| null hypothesis if pmin < α/m. It is now evident that the power | scheme when the comparison of forecasts is interpreted as be- |
| of this test can be driven to 0 by adding poor and irrelevant al- | ing conditional on the estimated parameters. An overview of |
| ternatives to the comparison, because this increases m but does | our notation is given in Table 1. This provides a general frame- |
| least as- not affect pmin. However, sample information will (at | work for comparing forecasts and decision rules. Our leading |
| ymptotically) identify the poor and irrelevant alternative, which | example is the comparison of forecasts, so we often refer to |
| allows us to use a smaller denominator when deﬁning the crit- | δk,t−h as the kth forecasting model. The ﬁrst model, k = 0, has |
| ical value, for example, α/m0 for some m0 ≤ m. Although our | a special role and is referred to as the benchmark. The decision |
| testing procedure is quite different from the conservative Bon- | rule, δk,t−h, can represent a point forecast, an interval forecast, |
| ferroni bound test, our sample-dependent null distribution is | a density forecasts, or a trading rule for an investor, as we illus- |
| similar to this improvement of the Bonferroni bound test, al- | trate next with some examples. |
| though the (presumed) poor alternatives are not discarded en- |  |
|  | Example 1 (Point forecast). Let δk,t−h, k = 0, 1, . . . , m, be |
| tirely in our framework. |  |
|  | different point forecasts of a real random variable ξt. The mean |
| In relation to the existing literature on forecast evaluation and |  |
|  | is an squared error loss function, L(ξt, δk,t−h) = (ξt − δk,t−h)2, |
| comparison, it is important to acknowledge a limitation of the |  |
|  | example of a loss function that could be used to compare the |
| speciﬁc test that we propose in this article. A comparison of |  |
|  | different forecasts. |
| models with parameters that are estimated recursively is not ac- |  |
| commodated by our framework, because this situation violates | Example 2 (Conditional distribution and value-at-risk fore- |
| our stationarity assumption. (For recent progress on this prob- | conditional density on R, |
|  | casts). Let be a and let ξt δk,t−h |
| lem in the present context, see Corradi and Swanson 2005a.) |  |
|  | be a forecast of evaluate the precision ξt. Then we might |
| However, our framework does permit parameters that are esti- | of by the Kolmogorov–Smirnov δk statistic, L(ξt, δk,t−h) = (cid:1) |
| mated once (ﬁxed scheme) or with a moving window (rolling | x− or a Kullback–Leibler ∞[ξt( y) − δk,t−h( y)] dy\|, supx∈R \| |
|  | (cid:1) ∞ |
| schemes), as we discuss in Section 2. The advantages of the |  |
|  | measure, L(ξt, δk,t−h) −∞ log[δk,t−h(x)/ξt(x)]ξt(x) dx. |
| studentized test statistic and our sample-dependent null distri- |  |
|  | Alternatively, could be a value-at-risk measure (at δk,t−h |
| bution do not rely on stationarity, so these modiﬁcations are |  |
|  | quantile α) that may be evaluated with L(ξt, δk,t−h) = |
|  | (cid:1) |
| expected to be useful in a more general context. A related issue | δk,t−h |
|  | \| |
|  | ξt(x) dx − α\|. −∞ |
| concerns the optimality of our test. Although the new test dom- |  |
| inates the RC, we do not claim that it is optimal. The lack of an | In Example 2, ξt will often be unobserved, which creates |
| optimality result is not surprising, because such results are rare | additional complications for empirical evaluation and compari- |
| in composite hypothesis testing. It is also worth observing that | son. When a proxy is substituted for ξt it can cause the empirical |
| leading statisticians continue to quarrel about what constitutes | ranking of alternatives to be inconsistent for the intended (true) |
| a suitable test in this context (see Perlman and Wu 1999 and | ranking (see Hansen and Lunde 2005a). Corradi and Swan- |
| the comments on that article by Berger, Cox, McDermott, and | son (2005b) recently derived an RC-type test for comparing |
| Wang). | conditional density forecasts, which is closely related to the |
| This article is organized as follows. Section 2 introduces the | problem of Example 2. Their test is similar to that of White |
| new test for SPA and contains our theoretical results. Section 3 | (2000), because their test statistic is also the maximum of mul- |
| provides the details of the bootstrap implementation. Section 4 | tiple nonstudentized quantities. So it would be interesting to |
| contains a simulation-based study of the ﬁnite-sample prop- | analyze whether our two modiﬁcations can be implemented in |
| erties of the new test for SPA and compares it with those of | their framework. |
| the RC. Section 5 contains an empirical forecasting exercise |  |
|  | Example 3 (Trading rules). Let δk,t−1 be a binary variable |
| of U.S. inﬂation, and Section 6 gives a summery and some con- |  |
|  | that instructs a trader to take either a short (δ = −1) or a long |
| cluding remarks. All proofs are presented in an Appendix. |  |
|  | (δ = 1) position in an asset at time t − 1. The kth trading rule |
|  | is the return on the yields the proﬁt πk,t = δk,t−1rt, where rt |
|  | asset in period t. A trader who is currently using the rule, δ0, |
| 2. TESTING FOR SUPERIOR PREDICTIVE ABILITY |  |
|  | might be interested to know whether an alternative rule has a |
| We consider a situation where a decision must be made h pe- | larger expected proﬁt than δ0. This can be formulated in our |
| riods in advance and let {δk,t−h, k = 0, 1, . . . , m} be a ﬁnite set | framework by setting ξt = rt and L(ξt, δk,t−1) = −δk,t−1ξt. |

Raw CSV: `assets/table_001.csv`

### Table 2

*Caption:* Table 1. Overview of Notation and Deﬁnitions

<table>
  <tr>
    <th>t = 1, . . . , n Sample period for the model comparison</th>
  </tr>
  <tr>
    <td>k = 0, 1, . . . , m Model index (k = 0 is the benchmark)</td>
  </tr>
  <tr>
    <td>Object (variable) of interest ξt</td>
  </tr>
  <tr>
    <td>The kth decision rule (e.g., h-step-ahead forecast of ξt) δk,t−h</td>
  </tr>
  <tr>
    <td>Observed loss of the kth decision rule/forecast Lk,t ≡ L(ξt, δk,t−h)</td>
  </tr>
  <tr>
    <td>Performance of model k relative to the benchmark dk,t ≡ L0,t − Lk,t</td>
  </tr>
  <tr>
    <td>(cid:2)</td>
  </tr>
  <tr>
    <td>nt Average relative performance of model k dk ≡ n−1</td>
  </tr>
  <tr>
    <td>=1 dk,t</td>
  </tr>
  ...
  <tr>
    <td>Vector of relative performances at time t dt ≡ (d1,t , . . . , dm,t)</td>
  </tr>
  <tr>
    <td>(cid:2)</td>
  </tr>
  <tr>
    <td>Vector of average relative performance d ≡ n−1</td>
  </tr>
  <tr>
    <td>nt =1 dt</td>
  </tr>
  <tr>
    <td>Expected excess performance of model k µk ≡ E(dk,t)</td>
  </tr>
  <tr>
    <td>(cid:5)</td>
  </tr>
  <tr>
    <td>Vector of expected excess performances µ ≡ (µ1, . . . , µm)</td>
  </tr>
  <tr>
    <td>(cid:1) ≡ avar(n1/2 ¯d) Asymptotic m× m covariance matrix</td>
  </tr>
  <tr>
    <td>(cid:2)</td>
  </tr>
  ...
  <tr>
    <td>nt The benchmark in Example 3 could be δ0,t = 1, which is the =1 dt and (cid:1) ≡ avar(n1/2( ¯d − µ)) (see, e.g.,</td>
  </tr>
  <tr>
    <td>rule that is always “long in the market.” This was the bench- de Jong 1997).</td>
  </tr>
  <tr>
    <td>mark used by Sullivan et al. (1999, 2001) who evaluated the Diebold and Mariano (1995) and West (1996) provided suf-</td>
  </tr>
  <tr>
    <td>signiﬁcance of technical trading rules and calendar effects in ﬁcient conditions that also lead to the asymptotic normality</td>
  </tr>
  <tr>
    <td>stock returns. in (2). Giacomini and White (2003) established this property</td>
  </tr>
  <tr>
    <td>for a related testing problem. However, the asymptotic normal-</td>
  </tr>
  <tr>
    <td>2.1 Hypothesis of Interest ity does not hold in general. An important exception is the sit-</td>
  </tr>
  <tr>
    <td>uation where the benchmark is nested in all alternative models</td>
  </tr>
  <tr>
    <td>We are interested to know whether any of the models,</td>
  </tr>
  <tr>
    <td>(under the null hypothesis) and the parameters are estimated</td>
  </tr>
  <tr>
    <td>k = 1, . . . , m, are better than the benchmark in terms of ex-</td>
  </tr>
  <tr>
    <td>recursively. In this situation the limiting distribution will typ-</td>
  </tr>
  <tr>
    <td>pected loss. So we seek a test of the null hypothesis that the</td>
  </tr>
  <tr>
    <td>ically be given as a function of Brownian motions (see, e.g.,</td>
  </tr>
  <tr>
    <td>benchmark is not inferior to any of the alternatives. The vari-</td>
  </tr>
  <tr>
    <td>Clark and McCracken 2001). When comparing nested models,</td>
  </tr>
  <tr>
    <td>ables that are key for our analysis are the relative performance</td>
  </tr>
  <tr>
    <td>the null hypothesis simpliﬁes to the simple hypothesis, µ = 0.</td>
  </tr>
  <tr>
    <td>variables, which are deﬁned by</td>
  </tr>
  <tr>
    <td>So in this case it seems more appropriate to apply a test for EPA,</td>
  </tr>
  <tr>
    <td>k = 1, . . . , m. dk,t ≡ L(ξt, δ0,t−h) − L(ξt, δk,t−h),</td>
  </tr>
  <tr>
    <td>such as that of Harvey and Newbold (2000), which can be used</td>
  </tr>
  <tr>
    <td>to compare multiple-nested models.</td>
  </tr>
  <tr>
    <td>denotes the performance of model k relative to the So dk,t</td>
  </tr>
  <tr>
    <td>At this point, all essential aspects of our framework are iden-</td>
  </tr>
  <tr>
    <td>benchmark at time t, and we stack these variables into the vector</td>
  </tr>
  <tr>
    <td>tical to those of White (2000). White proceeded by constructing of relative performances, dt = (d1,t, . . . , dm,t)(cid:5). Provided that</td>
  </tr>
  <tr>
    <td>the RC from the test statistic,</td>
  </tr>
  <tr>
    <td>µ ≡ E(dt) is well deﬁned, we can now formulate the null hy-</td>
  </tr>
  <tr>
    <td>pothesis of interest as (cid:4) (cid:3)</td>
  </tr>
  <tr>
    <td>, ≡ max T RC n1/2 ¯d1, . . . , n1/2 ¯dm</td>
  </tr>
  <tr>
    <td>n</td>
  </tr>
...
*Caption:* Table 2. Rejection Frequencies Under the Null and Alternative (m = 100 and n = 200)
...
| Hansen: A Test for Superior Predictive Ability | 373 |
| --- | --- |
| given by | where |
|  | − 1 λ2 λ3 ω2 and (cid:17) 1 + 1 µk = λk√ λk + 1 |
| 0 (cid:17) λ0 | k k k, |
|  | 2 4 12 n |
| 1 λ1 |  |
| 1 |  |
|         m−1 (cid:17)0 λ2 | now follows where the expression for ω2 from var(dk,t) = |
|  | k |
| 2 |  |
| λ3 = | var(L0,t − Lk,t) = 1 + var(Lk,t) |
| λ ≡ . m−1 (cid:17)0 | and the Taylor expansion 2 |
| . . |  |
| . . | (about 0) |
| . . | (cid:5) (cid:6) |
| m−2 |  |
| λm−1 m−1 (cid:17)0 | 1 2 . exp(arctan(x)) = 1 1 + x + 1 x2 − 1 x3 − 7 x4 + O(x5) |
| λm | 2 2 6 24 |
| (cid:17)0 |  |
| the In our experiments we use (cid:17)0 = 0, 1, 2, 5, 10 to control |  |
|  | 4.1 Simulation Results |
| extent to which the inequalities are binding with ((cid:17)0 = 0 cor- |  |
|  | First, we consider the case with m = 100 and the two sam- |
| responding to the case where all inequalities are binding). The |  |
|  | ple sizes n = 200 and n = 1,000. Then we consider the case |
| ﬁrst alternative model has (cid:17)1 = 0, −1, −2, −3, −4, −5. So |  |
|  | with m = 1,000 using the sample size n = 200. The rejection |
| the local alternative that is being analyzed λ1 = (cid:17)1 deﬁnes |  |
|  | frequencies that we report are based on 10,000 independent |
| (unless (cid:17)1 = 0, which conforms with the null hypothesis). To |  |
|  | samples, where we used q = 1 in accordance with the lack of |
| make the experiment more realistic, we tie the variance, σ 2 |  |
| k , to |  |
|  | time dependence in dt, t = 1, . . . , n. All of our simulations were |
| the “quality” of the model. Speciﬁcally, we set |  |
|  | made using Ox 3.30 (see Doornik 1999). The rejection frequen- |
| = 1 σ 2 |  |
| k 2 exp(arctan(λk)), | cies of the tests at the 5% and 10% levels are reported in Ta- |
|  | bles 2–4. The numbers in italic type are used when the null |
| such that a good model has a smaller variance than poor model. |  |
|  | hypothesis is true ((cid:17)1 = 0), so these frequencies correspond to |
| Note that this implies that |  |
| √ | type I errors. The numbers in regular type represent powers for |
| n¯dk ∼ N(µk, ω2 k ), | the various local alternatives ((cid:17)1 < 0). |
...
*Caption:* Table 4. Rejection Frequencies Under the Null and Alternative (m = 1,000 and n = 200)
...
| 376 | Journal of Business & Economic Statistics, October 2005 |
| --- | --- |
| make such an adjustment in practice without exceeding the in- | is car- estimation scheme ensures that stationarity of (Xt, Yt) |
| tended level of the test for other conﬁgurations—particularly | ried over to L(Yt+h, ˆβ(cid:5) |
|  | (k),tX(k),t), whereby a violation of As- |
| (See Horowitz and Savin 2000 the case where (cid:17)0 = (cid:17)1 = 0. | sumption 1 is avoided. For example, it is difﬁcult to reconcile |
| for a criticism of reporting “size”-adjusted powers.) From the |  |
|  | is estimated recursively Assumption 1 with the case where β(k) |
| power curves in Figure 4, it is clear that the RC is domi- | (i.e., using an expanding window of observation as n → ∞). |
| nated by the three other tests. There is a substantial increase | The ﬁrst forecast of annual inﬂation is made at time 1959:Q4 |
| in power from using the consistent distribution, and a similar | (predicting inﬂation for the year 1960:Q1–1961:Q1). So the |
| improvement is achieved by using the standardized test statis- | evaluation period includes n = 160 quarters: |
| tic, T SPA . For example, the local alternative (cid:17)1 = −4 is rejected |  |
| n | . t = 1952:Q1, . . . , 1959:Q4 , 1961:Q1, . . . , 2000:Q4 |
|  | (cid:19) (cid:19) (cid:20)(cid:21) (cid:22) (cid:20)(cid:21) (cid:22) |
| by the RC in about 5.5%. Using the data-dependent null distri- |  |
|  | initial estimation period evaluation period |
| bution (RCc) or the studentization (SPAu) improves the power |  |
| to about 73.6% and 96.4%. Invoking both modiﬁcations (as we | The models are evaluated using a mean absolute error crite- |
| advocate) improves the power to 99.7% in this simulation ex- | rion (MAE) given by L(Yt, ˆYk,t) = \|Yt − ˆYk,t\|, and the best- |
|  | performing models turn out to have a Phillips curve structure. In |
| periment. So both modiﬁcations are very useful, and the com- |  |
|  | fact, the best forecasts are produced by regressors that measure |
| bination of the two yields a substantial improvement in power. |  |
|  | (changes in) inﬂation, interest rates, employment, and GDP, and |
| Comparing the sample sizes that would result in the same |  |
|  | the very best sample performance is achieved by the three re- |
| power is an effective way to convey the relative efﬁciency of the |  |
|  | inﬂation, gressors X1,t, X8,t, and X13,t, which represent annual |
| tests. For the conﬁguration used in Figure 4, we see that the four |  |
| √ |  |
|  | employment relative to the previous year’s employment, and |
| n (cid:17) 2.13, tests have 50% power at the local alternatives, µ1/ |  |
|  | the change in GDP (see Table 5). We also include the aver- |
...
| sample-dependent null distribution. In other words, using the |  |
|  | 1999). In addition to the average forecast, the 27 regressors lead |
| LFC is equivalent to tossing away about 33% of the data. Sim- |  |
|  | to 3,303 regression-based forecasts when we consider all pos- |
| ilarly, dropping the studentization is equivalent to tossing away |  |
|  | sible subset regressions with 1, 2, or 3 regressors. So we are to |
| about 65% of the data, and dropping both modiﬁcations (i.e., |  |
|  | compare m = 3,304 forecasts to the random-walk benchmark, |
| is equivalent to tossing away using the RC instead of SPAc) |  |
|  | and we refer to this set of competing forecasts as the large uni- |
| about 84% of the data in this simulation design. |  |
|  | verse. |
|  | Panel A of Table 6 contains the output produced by the tests |
|  | for SPA for the large universe. Because the SPAc p value is .741, |
| 5. FORECASTING U.S. INFLATION USING LINEAR |  |
|  | there is no statistical evidence that any of the regression-based |
| REGRESSION MODELS |  |
|  | forecasts (including the average of them) are better than the |
|  | random-walk forecast. Note the discrepancy between the p val- |
| In an attempt to forecast annual U.S. inﬂation, we estimate |  |
|  | µl and µu. This difference suggests that some ues based on |
| a large number of linear regression models used to construct |  |
|  | of the alternatives are poor forecasts, and a closer inspection of |
| competing forecasts. The annual U.S. inﬂation rate is deﬁned |  |
|  | the large universe veriﬁes that several models have substantially |
| is the GDP price deﬂator for by Yt ≡ log[Pt/Pt−4], where Pt |  |
|  | worse performance than the benchmark. |
| the tth quarter. Inﬂation and most of the variables are not ob- |  |
|  | The ability to construct better forecasts using models with |
| served instantaneously. For this reason, we let the set of poten- |  |
|  | additional regressors is made difﬁcult by the need to estimate |
| tial regressors consist of variables that are lagged ﬁve quarters |  |
|  | additional parameters. In a forecasting exercise there is a trade- |
...
| 12-month period. |  |
|  | will (most likely) introduce a “bias,” but if this bias is small, it |
| The linear regression models include 1, 2, or 3 regressors out | may be less severe for out-of-sample predictions than the pre- |
| of the pool of 27 regressors, X1,t, . . . , X27,t, which leads to a | diction error introduced by estimation error (see, e.g., Clements |
| total of 3,303 regression models. Descriptions and deﬁnitions | and Hendry 1998). Exploiting this bias–variance trade-off is |
| of the regressors are given in Table 5. | particularly useful whenever the estimator is based on a mod- |
| The sequence of forecasts produced by the kth regression | erate number of observations, as is the case in our application. |
| model is given by | For this reason, we also consider a small universe of regression- |
|  | based forecasts, where all models include lagged inﬂation, X1,t, |
| τ = 0, . . . , n − 1, Yk,τ +5 ≡ ˆβ(cid:5) |  |
| (k),τ X(k),τ , | as a predictor (regressor) with a coefﬁcient set to unity. The |
|  | remaining parameters are estimated by ridge regressions that |
| contains the regressors included in model k where X(k),τ |  |
|  | shrink these parameters toward 0. |
| β(cid:5) is the least squares estimator based on the 32 and |  |
| (k),τ | Thus the regression models have the form |
| ˆ |  |
| ≡ |  |
| most recent observations (a rolling window). Thus β(k),τ |  |
|  | τ = 0, . . . , n − 1, Yτ +5 − Yτ ≡ β(cid:5) |
|  | (k)X(k),τ + ε(k),t, |
| the are given by rows of Xk,τ k,τ Xk,τ )−1X(cid:5) k,τ Yτ , where |  |
| X(cid:5) |  |
| (k),t−5, t = τ − 32 + 1, . . . , τ , and similarly the elements of Yτ | is a vector that includes either one or two regres- where X(k),τ |
| are given by Yτ , t = τ − 32 + 1, . . . , τ . Using a rolling-window | sors. As before, we use a rolling-window scheme (32 quar- |

Raw CSV: `assets/table_008.csv`

### Table 9

*Caption:* Table 5. Deﬁnitions of Variables

| Panel A: Description of variables |  |  |
| --- | --- | --- |
| Annual inﬂation Yt |  |  |
| Annual inﬂation (lags of Yt) X1, t, X2, t |  |  |
| Quarterly inﬂation X3, t, X4, t |  |  |
| Quarterly inﬂation relative to previous year’s inﬂation X5, t |  |  |
| Changes in employment in manufacturing sector X6, t, X7, t |  |  |
| Quarterly employment relative to average of previous year X8, t |  |  |
| Quarterly employment relative to average of previous 2 years X9, t |  |  |
| Quarterly changes in real inventory X10, t, X11, t |  |  |
| Quarterly changes in quarterly GDP X12, t, X13, t |  |  |
| Interest paid on 3-month T-bill X14, t |  |  |
| Changes in 3-month T-bill X15, t, X16, t |  |  |
| Changes in 3-month T-bill relative to level of T-bill X17, t, X18, t |  |  |
| Changes in prices of fuel and energy X19, t, X20, t |  |  |
| Changes in prices of food X21, t, X22, t |  |  |
| Quarterly dummies: ﬁrst, second, third, and fourth quarters X23, t–X26, t |  |  |
| Constant X27, t |  |  |
| Panel B: Deﬁnitions of variables |  |  |
| Yt = log(GDPCTPIt) − log(GDPCTPIt − 4), X1, t = Yt − 5, X2, t = Yt − 8 |  |  |
| X3, t = 4[log(GDPCTPIt) − log(GDPCTPIt − 1], X4, t = X3, t − 1 |  |  |
| X5, t = log(1 + X3, t) − log(1 + X1, t − 1) |  |  |
| X6, t = log(MANEMPt) − log(MANEMPt − 1), X7, t = X6, t − 1 |  |  |
| (cid:2) |  |  |
| 4i = log(MANEMPt) − log( 1 X8, t =1 MANEMPt − i) 4 |  |  |
| (cid:2) |  |  |
| 8i = log(MANEMPt) − log( 1 X9, t =1 MANEMPt − i) 8 |  |  |
| X10, t = log(CBIt) − log(GDPt), X11, t = X10, t − 1 |  |  |
| X12, t = log(GDPt) − log(GDPt − 1), X13, t = X12, t − 1 |  |  |
| X14, t = TB3MSt, X15, t = (cid:20)X14, t, X16, t = X15, t − 1, X17, t = (cid:20)X14, t/X14, t, | X18, t = X17, t − 1 |  |
| X19, t = log(PPIENGt) − log(PPIENGt − 1), X20, t = X19, t − 1 |  |  |
| X21, t = log(PPIFCFt) − log(PPIFCFt − 1), X22, t = X21, t − 1 |  |  |
| X23, t = 1, X24, t = X23, t − 1, X25, t = X23, t − 2, X26, t = X23, t − 3, X27, t = 1 |  |  |
| Raw data: GDPCTPI = gross domestic product: chain-type price index; CBI = change in private inventories; GDP = gross domes- |  |  |
| ∗ |  |  |
| tic product; TB3MS = 3-month Treasury bill rate, secondary market ; PPIENG = producer price index: | fuels and related products |  |
| ∗∗ ∗∗ |  |  |
| and power ; PPIFCF = producer price index: ﬁnished consumer foods | ; MANEMP = employees on nonfarm payrolls: manufac- |  |
| turing. |  |  |
| ∗ Quarterly data are deﬁned as the average of the monthly observations over the quarter. |  |  |
| ∗∗ Quarterly data are deﬁned as be the last monthly observation of the quarter. |  |  |
| Table6. TestsforSuperiorPredictiveAbility |  |  |
| Loss | t-statistic | pvalue |
| Panel A: Results for the large universe of forecasts |  |  |
...
*Caption:* Table 6. Tests for Superior Predictive Ability
...
| Evaluated by MAE | Benchmark: .0098 |  |
| m= 3,304 (number of models) | Best performing: .0084 1.2363 | .120 |
| n= 160 (sample size) | Most signiﬁcant: .0085 1.2628 | .112 |
| B= 10,000 (resamples) | Median: .0141 −2.7694 |  |
| q= .25 (dependence) | Worst: .0416 −7.8939 |  |
| RCl | RCc RCu SPAc SPAl | SPAu |
| SPA p values .503 | .781 .978 .571 .741 | .903 |
| Panel B: Results for the small universe of forecasts |  |  |
| Evaluated by MAE | Benchmark: .0098 |  |
| m= 352 (number of models) | Best performing: .0082 2.7547 | .006 |
| n= 160 (sample size) | Most signiﬁcant: .0096 2.9399 | .004 |
| B= 10,000 (resamples) | Median: .0097 .0657 |  |
| q= .25 (dependence) | Worst: .0107 −1.3272 |  |
| RCl | RCc RCu SPAc SPAl | SPAu |
| SPA p values .071 | .106 .106 .045 .048 | .048 |
| Panel C: Results for the full universe of forecasts |  |  |
| Evaluated by MAE | Benchmark: .0098 |  |
...
| q= .25 (dependence) | Worst: .0416 −7.8939 |  |
| RCl | RCc RCu SPAc SPAl | SPAu |
| SPA p value .395 | .691 .963 .078 .100 | .135 |
| NOTE: The table reports SPA p values for three sets of | regression-based forecasts that are compared to a random-walk forecast. The p value of the new test, SPAc, | is in bold type. Panel A |
| contains the results for the large universe, panel B contains the results for | the small universe, and panel C contains the results for the full universe. For each “universe of | forecasts” we also |
| report the sample loss for the benchmark and the four alternative forecasts that had the smallest sample loss, | the largest t-statistic for relative sample loss ( ¯dk), | the median sample loss (across |
| alternatives), and the worst sample loss. The corresponding t-statistic (of | their sample loss relative to the benchmark) is given in the second last column. We also report | the “p values” from the |
| pairwise comparisons of “best” and “largest t-statistic” forecasts to the benchmark. These p values (unlike the SPA p values) do not account for the entire universe of forecasts. |  |  |
...
| 378 | Journal of Business & Economic Statistics, October 2005 |
| --- | --- |
| ˆ |  |
| ≡ |  |
| ters), but with the estimator for β(k) now given by β(k),τ | irrelevant alternatives and reduce their inﬂuence on the test |
| ˜ |  |
| (X(cid:5) | for SPA. |
| Yτ , where λ = .1 is the shrinkage pa- k,τ Xk,τ + λI)−1X(cid:5) k,τ |  |
| ˜ | The power improvements were quantiﬁed in simulation ex- |
| for rameter and the elements of Yτ are given by Yt − Yt−5 |  |
|  | periments and an empirical forecasting exercise of U.S. inﬂa- |
| t = τ − 32 + 1, . . . , τ . This results in 351 regression-based fore- |  |
|  | tion. These also highlighted that the RC is sensitive to poor |
| casts plus the average forecast, such that the total number of al- |  |
|  | and irrelevant alternatives. Two researchers are therefore more |
| ternative forecasts in the small universe is m = 352. The most |  |
|  | likely to arrive at the same conclusion when they use the |
| accurate forecast in the small universe is produced by the re- |  |
|  | test than they would when using the RC—even if they SPAc |
| gression model with the regressors X8,t and X9,t, which are |  |
|  | do not fully agree on the set of forecasts that is relevant for the |
| two measures of (relative) employment. The largest t-statistic |  |
|  | analysis. |
| is produced by the regressions X6,t and X10,t, which represent |  |
|  | Interestingly, we found that the best (and most signiﬁcant) |
| changes in employment and inventories. So our ﬁndings sup- |  |
|  | predictions of U.S. inﬂation were produced by regression-based |
| port a conclusion reached by Stock and Watson (1999) that |  |
|  | forecasts that had a Phillips curve structure. In our full uni- |
| forecasts based on Phillips curve speciﬁcations are useful for |  |
|  | verse of alternatives, we found that the (random-walk) bench- |
| forecasting inﬂation. |  |
|  | mark forecast is outperformed by the regression-based forecasts |
| The empirical results for this universe are presented in |  |
|  | if a moderate (10%) signiﬁcance level is used. Whereas the |
| p value for this universe is panel B of Table 6. The SPAc |  |
|  | test yields a p value of 10%, the RC yields a p value of SPAc |
| .048, which suggests that the benchmark is outperformed by |  |
|  | about 96%, such that the two tests arrive at opposite conclusions |
| the regression-based forecasts. For each of the test statistics, |  |
|  | (weak evidence against H0 vs. no evidence). This is caused by |
...
| than the benchmark, such that µl and ˆµu are similar. The differ- |  |
|  | discussed Hansen and Lunde (2005b), who compared a large |
| ence in p values across the two test statistics is fairly modest but |  |
|  | number of volatility models using the methods of this article. |
| do suggest some variation in the variances, ω2 k , k = 1, . . . , 352. |  |
|  | Although there are several advantages of our new test, some |
| Reporting the results in panel B is not fully consistent with |  |
|  | important issues need to be addressed in future research. In this |
| the spirit of this article, because the results in panel B do not |  |
...
| of data mining. To address this concern, we perform the tests |  |
|  | construct a suitable test that can accommodate this situation and |
| for SPA over the union of the large universe and the small uni- |  |
|  | analyze the need for our two modiﬁcations in this framework. |
| verse. We refer to this set of forecasts as the full universe, and |  |
|  | Despite its many pitfalls, data mining is a constructive device |
| present the results for this set of alternatives in panel C. Adding |  |
|  | for the discovery of true phenomena and has become a popu- |
| the large number of insigniﬁcant alternatives to the comparison |  |
|  | lar tool in many applied areas, such as genetics, e-commerce, |
| reduces the signiﬁcance, although the excess performance con- |  |
...
|  | the bar at which alternatives are classiﬁed as being signiﬁcantly |
| the p value is most likely due to the RC’s sensitivity to poor and |  |
|  | better than the benchmark. This aspect is particularly problem- |
| test’s p value increases from irrelevant alternatives. The SPAc |  |
|  | atic for economic applications where data are scarce. In this |
| 4.8% to 10%. Although this increment is more moderate, it re- |  |
|  | context it is particularly useful to conﬁne the exploration to al- |
| veals that the new test is not entirely immune to the inclusion |  |
|  | ternatives motivated by theoretical considerations. Our empiri- |
| of (a large number of ) poor forecasts. This reminds us that ex- |  |
|  | cal application provides a good illustration of this issue. Within |
| cessive data mining can be costly in terms of the conclusions |  |
|  | the small universe we found fairly compelling evidence against |
| that can be drawn from an empirical analysis, because it may |  |
|  | the null hypothesis, and ex post it is easy to produce arguments |
...
|  | that motivate the use of shrinkage methods, which led to the |
| is signiﬁcant. Given the scarcity of macroeconomic data, it will |  |
| often be useful to conﬁne the set of alternatives to those mo- | small universe of regression-based forecasts. However, because |
| tivated by theoretical considerations, instead of undertaking a | the large universe was explored in the initial analysis, we can- |
| blind search over a large number of alternatives. | not exclude the possibility that the largest t-statistic would have |
|  | been found in this universe. The weaker evidence against the |
|  | null hypothesis found in the full universe is the price that we |
| 6. SUMMARY AND CONCLUDING REMARKS |  |
|  | have to pay for the data exploration that preceded our analysis |
|  | of the small universe. |
| We have analyzed the problem of comparing multiple fore- |  |
| casts to a given benchmark through tests for superior predic- |  |
...
![Figure 17: Figure 1. (One minus) The cdfs for the Test Statistics T RC and TSPA Under the Null Hypothesis, µ1 = µ2 = 0, and the Local Alternative,](assets/fig_017.png)

![Figure 18: Figure 2. A Situation Where the RC Fails to Reject a False Null Hyn1/2(¯dk −µk)](assets/fig_018.png)

![Figure 19: Figure 3 shows how the consistent estimate of the null disthe three-step method of Andrews and Buchinsky (2000), can](assets/fig_019.png)

![Figure 20: Figure 4. Local Power Curves of the Four Tests, SPAc, SPAu, RCc, and RCu, for the Simulation Experiment Where m = 100, Λ0 = 20, and](assets/fig_020.png)

## Extraction Notes

- discarded low-quality embedded figure on page 5: score=0.00, flags=small_area,small_edge,extreme_aspect_ratio,small_payload,caption_nearby
- discarded low-quality embedded figure on page 5: score=0.00, flags=small_area,small_edge,extreme_aspect_ratio,small_payload,caption_nearby
- discarded low-quality embedded figure on page 5: score=0.00, flags=small_area,small_edge,extreme_aspect_ratio,small_payload,caption_nearby
- discarded low-quality embedded figure on page 5: score=0.00, flags=small_area,small_edge,extreme_aspect_ratio,small_payload,caption_nearby
