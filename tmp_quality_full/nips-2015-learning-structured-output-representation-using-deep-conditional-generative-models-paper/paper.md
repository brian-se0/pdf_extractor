---
paper_id: "nips-2015-learning-structured-output-representation-using-deep-conditional-generative-models-paper"
source_file: "NIPS-2015-learning-structured-output-representation-using-deep-conditional-generative-models-Paper.pdf"
title: "Learning Structured Output Representation using Deep Conditional Generative Models"
authors: ["Kihyuk Sohn, Honglak Lee, Xinchen Yan"]
year: null
doi: null
page_count: 9
extracted_at: "2026-02-07T05:35:20.547134+00:00"
status: "success"
---
...
- **Source File:** `NIPS-2015-learning-structured-output-representation-using-deep-conditional-generative-models-Paper.pdf`
- **Authors:** Kihyuk Sohn, Honglak Lee, Xinchen Yan
- **Year:** Unknown
- **DOI:** Unknown
...
The model is trained efﬁciently in the framework of stochastic gradient variational Bayes, and allows for fast prediction using stochastic feed-forward inference. In addition, we provide novel strategies to build robust structured prediction
...
The paper is organized as follows. We ﬁrst review related work in Section 2. We provide preliminaries in Section 3 and develop our deep conditional generative model in Section 4. In Section 5,
...
signiﬁcantly enhanced this line of research [2, 7, 8, 18]. In particular, the variational learning
...
posterior inference. However, the parameters of the VAE can be estimated efﬁciently in the stochastic gradient variational Bayes (SGVB) [16] framework, where the variational lower bound of the
...
L
eLVAE(x; θ, φ) = −KL (qφ(z|x)∥pθ(z)) + 1
X
log pθ(x|z(l)),
(3)
L
l=1
where z(l) = gφ(x, ϵ(l)), ϵ(l) ∼N(0, I). Note that the recognition distribution qφ(z|x) is reparameterized with a deterministic, differentiable function gφ(·, ·), whose arguments are data x and
the noise variable ϵ. This trick allows error backpropagation through the Gaussian latent variables,
...
L
eLCVAE(x, y; θ, φ) = −KL (qφ(z|x, y)∥pθ(z|x)) + 1
X
log pθ(y|x, z(l)),
(5)
L
l=1
where z(l) = gφ(x, y, ϵ(l)), ϵ(l) ∼N(0, I) and L is the number of samples. We call this model
...
inference without sampling z, i.e., y∗= arg maxy pθ(y|x, z∗), z∗= E
.2


z|x
...
L
eLGSNN(x, y; θ, φ) = 1
log pθ(y|x, z(l)) , where z(l) = gθ(x, ϵ(l)), ϵ(l) ∼N(0, I)
X
(8)
L
l=1
...
3If we assume a covariance matrix of auxiliary Gaussian latent variables ϵ to 0, we have a deterministic
...
X
more challenging to make a ﬁne-grained pixel-level predic1
...
1/4
1/2
...
training [17, 29]. Similarly, we propose a simple regularization technique for semantic segmentation: corrupt the input data x into ˜x according to noise process and optimize the network with the
following objective: eL(˜x, y). The noise process could be arbitrary, but for semantic image segmentation, we consider random block omission noise. Speciﬁcally, we randomly generate a squared
...
output variables. For the proof of concept, we create an artiﬁcial experimental setting for structured output prediction using MNIST database [19]. Then, we evaluate the proposed CVAE models
...
5.1
Toy example: MNIST
To highlight the importance of probabilistic inference through stochastic neurons for structured output variables, we perform an experiment using MNIST database. Speciﬁcally, we divide each digit
...
of the CLL. We observed that the estimated CLLs of the CVAE signiﬁcantly outperforms the baseline NN. Moreover, as measured by the per pixel performance gap, the performance improvement
...
Speciﬁcally, we report the performance of the models with different network architectures and training methods (e.g., multi-scale prediction or noise-injection training).
...
for post-processing) [37] even without post-processing. On top of that, we observed signiﬁcant performance improvement with our proposed deep CGMs.5 In terms of prediction accuracy, the GSNN
...
for inference.6 Finally, the performance of our two winning entries (GSNN and hybrid) on the validation sets are both signiﬁcantly better than their deterministic counterparts (GDNN) with p-values
...
database [12] has been widely used for face recognition and
...
although the accuracies of the CGM variants are higher, the performance of GDNN was not signiﬁcantly behind than those of GSNN and hybrid models. This may be because the level of variations in
the output space of LFW database is less than that of CUB database as the face shapes are more similar and better aligned across examples. Finally, our methods signiﬁcantly outperform other existing
...
98.73
8
information to iteratively reﬁne the predic74.95
95.95
91.84
97.29
1
...
97.08
4
ated samples at noise level of 50% in Fig76.07
89.10
92.68
96.15
8
...
CVAE can also be potentially useful for inCUB and LFW database. We report the pixel-level acteractive segmentation (i.e., by iteratively
...
Acknowledgments
This work was supported in part by ONR grant N00014-13-1-0762 and NSF
...
*Caption:* Table 1: The negative CLL on MNIST database. We increase the number of quadrants for an input
...
| 1quadrant |  | 2quadrants |  |  |
| --- | --- | --- | --- | --- |
| validation | test | validation | test | validation |
| 100.03 100.03 68.62 64.05 | 99.75 99.82 68.39 63.91 | 62.14 62.48 45.57 44.96 | 62.18 62.41 45.34 44.73 | 26.01 26.20 20.97 20.97 |
| 35.98 0.061 | 35.91 0.061 | 17.51 0.045 | 17.68 0.045 | 5.23 0.027 |
...
*Caption:* Table 2: Mean and standard error of labeling accuracy on CUB and LFW database. The performance

| CUB(val) |  | CUB(test) |  |  |
| --- | --- | --- | --- | --- |
| pixel | IoU | pixel | IoU | pixel(val) |
| – – | – – | 90.42 – | 75.92 – | – – |
| 91.17±0.09 91.37±0.09 | 79.64±0.24 80.09±0.25 | 92.30 92.52 | 81.90 82.43 | 92.09±0.13 92.19±0.10 |
| 92.25±0.09 92.46±0.07 92.24±0.09 92.60±0.08 | 81.89±0.21 82.31±0.19 81.86±0.23 82.57±0.26 | 93.24 93.39 93.03 93.35 | 83.96 84.26 83.53 84.16 | 92.72±0.12 92.88±0.08 92.80±0.30 92.95±0.21 |
| 92.92±0.07 93.09±0.09 92.72±0.08 93.05±0.07 | 83.20±0.19 83.62±0.21 82.90±0.22 83.49±0.19 | 93.78 93.91 93.48 93.78 | 85.07 85.39 84.47 85.07 | 93.59±0.12 93.71±0.09 93.29±0.17 93.69±0.12 |

Raw CSV: `assets/table_004.csv`

### Table 5

*Caption:* Table 3: Mean and standard error of negative CLL on CUB and LFW database. The performance of

| CUB(val) | CUB(test) | LFW(val) |
| --- | --- | --- |
| 4269.43±130.90 3386.19±44.11 3400.24±59.42 801.48±4.34 1019.93±8.46 | 4329.94±91.71 3450.41±33.36 3461.87±25.57 801.31±1.86 1021.44±4.81 | 6370.63±790.53 4710.46±192.77 4582.96±225.62 1262.98±64.43 1836.98±127.53 |

Raw CSV: `assets/table_005.csv`

### Table 6

*Caption:* Table 4: Segmentation results with omission noise on

|  | CUB(IoU) |  |  |
| --- | --- | --- | --- |
| block size | GDNN | CVAE | GDNN |
| 1 4 8 | 89.37 88.74 90.72 | 98.52 98.07 96.78 | 96.93 96.55 97.14 |
| 1 4 8 | 74.95 70.48 76.07 | 95.95 94.25 89.10 | 91.84 90.87 92.68 |
| 1 4 8 | 62.11 57.68 63.59 | 89.44 84.36 76.87 | 85.27 85.70 87.83 |

Raw CSV: `assets/table_006.csv`

## Figures

![Figure 1: Figure 1: Illustration of the conditional graphical models (CGMs). (a) the predictive process of](assets/fig_001.jpeg)

![Figure 2: Figure 2: Multi-scale prediction.](assets/fig_002.png)

![Figure 3: Figure 3: Visualization of generated samples with (left) 1 quadrant and (right) 2 quadrants for an](assets/fig_003.jpeg)

![Figure 4: Figure 3: Visualization of generated samples with (left) 1 quadrant and (right) 2 quadrants for an](assets/fig_004.jpeg)

![Figure 5: Figure 4: Visualization of the conditionally generated samples: (ﬁrst row) input image with omission](assets/fig_005.jpeg)

![Figure 6: Figure 4: Visualization of the conditionally generated samples: (ﬁrst row) input image with omission](assets/fig_006.jpeg)

![Figure 7: Figure 4: Visualization of the conditionally generated samples: (ﬁrst row) input image with omission](assets/fig_007.jpeg)

![Figure 8: Figure 4: Visualization of the conditionally generated samples: (ﬁrst row) input image with omission](assets/fig_008.jpeg)

![Figure 9: Figure 4: Visualization of the conditionally generated samples: (ﬁrst row) input image with omission](assets/fig_009.jpeg)

![Figure 10: Figure 4: Visualization of the conditionally generated samples: (ﬁrst row) input image with omission](assets/fig_010.jpeg)

![Figure 11: Figure 4: Visualization of the conditionally generated samples: (ﬁrst row) input image with omission](assets/fig_011.jpeg)

![Figure 12: Figure 4: Visualization of the conditionally generated samples: (ﬁrst row) input image with omission](assets/fig_012.jpeg)

## Extraction Notes

- pdfplumber unusable table on page 4 index 3
- pdfplumber unusable table on page 4 index 4
- pdfplumber unusable table on page 5 index 2
- pdfplumber table quality filter dropped 5 low-quality table(s)
